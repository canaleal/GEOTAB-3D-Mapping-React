...
const Home = () => {
  ...
  useEffect(() => {
    ...
    setLayers([{ "id": 1, "layer": 'Buildings', "isOn": true, "isDynamic": false, "layerName": "BuildingsLayer", "imgPath": "Buildings.JPG", "showButton": true, "icon": "fa-building" },
    { "id": 2, "layer": 'City Boundary', "isOn": true, "isDynamic": false, "layerName": "CityBoundaryLayer", "imgPath": "Boundary.JPG", "showButton": true, "icon": "fa-border-all" },
    { "id": 3, "layer": 'Pedestrians', "isOn": true, "isDynamic": false, "layerName": "PedestriansLayer", "imgPath": "Pedestrians.JPG", "showButton": true, "icon": "fa-person" },
    { "id": 4, "layer": 'Bus Routes', "isOn": false, "isDynamic": false, "layerName": "BuseRoutesLayer", "imgPath": "BusRoutes.JPG", "showButton": false, "icon": "fa-bus" },
    { "id": 5, "layer": 'City Projects', "isOn": false, "isDynamic": true, "layerName": "CityPlanningLayer", "imgPath": "CityProjects.JPG", "showButton": false, "icon": "fa-building-columns" },
    ])
        ...
        <div className="grid grid-cols-4 grid-row-3 gap-4 ">
          ...
          <div className='col-span-4 md:col-span-1  border bg-white rounded-lg p-4 slide-in-left'>
            <p className="font-bold">Layers</p>
            {
              layers.map((item) =>
                <span key={item.id}>
                  {item.showButton ? <LayerButton item={item} layerHandler={layerHandler} /> : <span></span>}
                </span>
              )
            }

            <button onClick={() => showModalHandler(layerModalRef)} className={`border block w-full text-sm text-left p-3 my-1 mt-10 rounded-md border-black-500 bg-slate-0 hover:bg-slate-200`}  >
              <FontAwesomeIcon icon="fa-solid fa-layer-group" size='lg' width={"2rem"} />Add Layers
            </button>
          </div>
          ...
          <div className='col-span-4 md:col-span-3 row-span-3 border bg-white rounded-lg h-[32rem] md:h-full slide-in-right relative' >
            ...
            <div className="absolute bottom-10 px-20 box-border  w-full">
              <div className="bg-black px-10 py-5  rounded-lg">
                <TimeSlider minYear={years[0]} maxYear={years[years.length - 1]} currentYear={currentYear} yearSliderHandler={yearSliderHandler} />
              </div>
            </div>
